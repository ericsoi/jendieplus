<?php
    ...
    // if ($success == "Success. Request accepted for processing"){
    //     // sleep(30);
    //     $select = $pdo->prepare("SELECT * from tbl_mpesa where date >= NOW() - interval 2 minute and MSISDN = '$phone'");
    //     $select->execute();
    //     $row = $select->fetch(PDO::FETCH_ASSOC);
    ...
    //         $_SESSION["stk_callback"] = $row;
            // include "b2b.php";
            // sleep(6);
            // $underwriter_select = $pdo->prepare("SELECT * from underwriters where Name = '$underwriter'");
            // $underwriter_select->execute();
            // $underwriter_row = $underwriter_select->fetch(PDO::FETCH_ASSOC);
            // $underwriter_code = trim($underwriter_row["paybill"]);
            // $select = $pdo->prepare("SELECT * from tbl_b2b_mpesa_transactions where date >= NOW() - interval 2 minute and system_underwriter = '$underwriter_code'");
            // $select->execute();
            // $row = $select->fetch(PDO::FETCH_ASSOC);
            // if(!empty($row)){
                // $responce = $success . " " . $phone;
        ...
        // sleep(15);
        // $select = $pdo->prepare("SELECT * from tbl_mpesa_transactions where date >= NOW() - interval 2 minute and MSISDN = '$phone'");
        // $select->execute();
        // $row = $select->fetch(PDO::FETCH_ASSOC);
        #print_r($row);
    ...
    //                         $wallet = "UPDATE tbl_user SET wallet = wallet + '$commission' WHERE username = '$user'";
