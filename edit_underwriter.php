    ...
    if (isset($_SESSION["username"])){
        ...
        if($user){
            ...
            if($user_info->role == "admin" && $user_info->is_active==1){
                $underwriter=selectuser("SELECT * FROM tbl_email WHERE ID='$ID'");
                $Name=$underwriter->Name;
                $EMAIL_ADDRESS=$underwriter->EMAIL_ADDRESS;
                $description=$underwriter->description;
                $agentcode=$underwriter->agencycode;
            
                if(isset($_POST["submit"])){
                    $EMAIL_ADDRESS = $_POST['email'];
                    $EMAIL_CC = $_POST['emailcc'];
                    $description = $_POST['desc'];
                    $agentcode = $POST ['agencycode'];
                    $update = $pdo->prepare("UPDATE tbl_email SET EMAIL_ADDRESS='$EMAIL_ADDRESS', EMAIL_CC='$EMAIL_CC', agencycode='$agentcode,  WHERE ID='$ID'");
                    $update->execute();
            ...
            elseif($user_info->role == "agency" && $user_info->is_active==1){
                $underwriter=selectuser("SELECT * FROM tbl_email WHERE ID='$ID'");
                $Name=$underwriter->underwriter;
                
                
                if(isset($_POST["submit"])){
                    $EMAIL_ADDRESS = $_POST['email'];
                    $EMAIL_CC = $_POST['cc'];
                    $agentcode = $_POST['code'];
                    $description = $_POST['desc'];
                    $update = $pdo->prepare("UPDATE tbl_email  SET  email='$EMAIL_ADDRESS',emailcc='$EMAIL_CC',agencycode='$agentcode',description='$description' WHERE id='$ID'");
                    $update->execute();
                    if($update->rowCount() > 0){
                    }elseif($update->execute()){
                    }
                                    ...
                                    <div class="">
                                        <div class="">
                                            <form role="form" action="#" method="post">
                                                ...
                                                <div class="mb-8">
                                                    <label class="mb-2 ml-1 font-bold text-size-xs text-slate-700">API Agency Code</label>
                                                    <input type="text"  name="code" class="focus:shadow-soft-primary-outline text-size-sm leading-5.6 ease-soft block w-half appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding  py-2 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:outline-none focus:transition-shadow"
                                                    value="<?php echo $agentcode;?>" placeholder="<?php echo $agentcode;?>" aria-label="code" aria-describedby="text-addon" />
                                                    <label class="mb-2 ml-1 font-bold text-size-xs text-slate-700">Email</label>
                                                    <input type="email"  name="email" class="focus:shadow-soft-primary-outline text-size-sm leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding  py-2 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:outline-none focus:transition-shadow"
                                                    value="<?php echo $EMAIL_ADDRESS;?>" placeholder="<?php echo $EMAIL_ADDRESS;?>" aria-label="Email" aria-describedby="email-addon" />
                                                    
                                                    <label class="mb-2 ml-1 font-bold text-size-xs text-slate-700">Email CC, single or multiple emails comma seperated</label>
                                                    <input type="text"  name="cc" class="focus:shadow-soft-primary-outline text-size-sm leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding  py-2 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:outline-none focus:transition-shadow"
                                                    value="<?php echo $EMAIL_CC;?>" placeholder="<?php echo $EMAIL_CC;?>" aria-label="Email" aria-describedby="email-addon" />

                                                    <input type="hidden"  name="name" class="focus:shadow-soft-primary-outline text-size-sm leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding  py-2 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:outline-none focus:transition-shadow"
                                                     placeholder="<?php echo $EMAIL_ADDRESS;?>" aria-label="Email" aria-describedby="email-addon" />
                                                    <input type="hidden"  name="desc" class="focus:shadow-soft-primary-outline text-size-sm leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding  py-2 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:outline-none focus:transition-shadow"
                                                     placeholder="<?php echo $EMAIL_CC;?>" aria-label="Email" aria-describedby="email-addon" />
                                                </div>
                                                <?php if($user_info->role==="admin"){?>
                                                    <div class="mb-12">
                                                        <label class="mb-2 ml-1 font-bold text-size-xs text-slate-700">Description</label>
                                                        <textarea type="text" name="desc" class="focus:shadow-soft-primary-outline text-size-sm leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding  py-2 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:outline-none focus:transition-shadow"
                                                        placeholder="<?php echo $description;?>"  aria-label="Email" aria-describedby="email-addon"></textarea>
                                                </div>
                                                <?php 
                                                }
                                                ?>
                                                ...
                                                <input type="button"  onclick="window.location='underwriters.php';"  class="inline-block px-2 py-1 font-bold text-right text-red align-middle transition-all  rounded-lg cursor-pointer leading-pro text-size-xs ease-soft-in shadow-soft-md bg-150 bg-gradient-dark-gray hover:shadow-soft-xs active:opacity-85 hover:scale-102 tracking-tight-soft bg-x-25" value="Back"/></i>
                                            
                                                <button type="submit" name="submit" id="submit" class="inline-block px-2 py-1 font-bold text-right text-green align-middle transition-all rounded-lg cursor-pointer leading-pro text-size-xs ease-soft-in shadow-soft-md bg-150 bg-gradient-dark-gray hover:shadow-soft-xs active:opacity-85 hover:scale-102 tracking-tight-soft bg-x-25"></i>Submit </button>
                                                </div> 
                                            </form>
                                        </div>
                                    </div>
            ...
            <div class="container">
                <div class="flex flex-wrap -mx-3">
                    <div class="flex-shrink-0 w-full max-w-full mx-auto mb-6 text-center lg:flex-0 lg:w-8/12">
                        <a href="#" target="#" class="mb-2 mr-4 text-slate-400 sm:mb-0 xl:mr-12"> Home</a>
