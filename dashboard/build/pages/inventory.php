<?php
    ...
    if (isset($_SESSION["username"])){
        ...
        $select = $pdo->prepare("SELECT Name FROM tbl_underwriter");
        $select->execute();
        $name = [];
        while ($names = $select->fetch(PDO::FETCH_ASSOC)){
            array_push($name, trim($names['Name']));
        };
        if($user){
            $user_info=$user;
                        ...
                        <div class="max-w-full px-3 mb-4 lg:mb-0 lg:w-full lg:flex-none">
                            <div class="relative flex flex-col min-w-0 mt-6 break-words bg-white border-0 border-transparent border-solid shadow-soft-xl rounded-2xl bg-clip-border">
                                <div class="p-4 pb-0 mb-0 bg-white border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
                                    <div class="flex flex-wrap -mx-3">
                                        <div class="flex items-center flex-none w-1/2 max-w-full px-3">
                                            <h6 class="mb-0">Member Company Inventory</h6>
                                        </div>
                                ...
                                <div class="flex-auto p-4">
                                    <div class="">
                                        <div class="">
                                            <form role="form" action="processor/handle_inventory_search.php" method="post">
                                                
                                                <div class="mb-8">
                                                    <label class="mb-2 ml-1 font-bold text-size-xs text-slate-700">Underwriting Company</label>
                                                    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/jqueryui/1.12.1/jquery-ui.css" />
                                                    <script src="https://code.jquery.com/jquery-3.4.1.min.js"></script>
                                                    <script src="https://code.jquery.com/ui/1.12.1/jquery-ui.min.js"></script>
                                                    <input type="text"  id="term" name="term" class="focus:shadow-soft-primary-outline text-size-sm leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding  py-2 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:outline-none focus:transition-shadow"
                                                        placeholder="Enter Underwriter Name" aria-label="Email" aria-describedby="email-addon" onchange="handle_search(this)"  required/>
                                                    <script type="text/javascript">
                                                    $(function() {
                                                        $( "#term" ).autocomplete({
                                                        source: 'processor/underwriter_search.php',
                                                        });
                                                    });
                                                    </script>	
                                                        ...
                                                        <div class="overflow-hidden">
                                                            ...
                                                            <?php
                                                                if(isset($_SESSION['inventory'])){
                                                                    echo $_SESSION['inventory_underwriter'];
                                                                    $counter = 0;
                                                                    $data = $_SESSION['inventory'];
                                                                    ...
                                                                    <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                                                                        <?php echo $data[$counter]['CertificateClassificationID'];?>
                                                                    </td>
                                                                    <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                                                                        <?php echo $data[$counter]['ClassificationTitle'];?>
                                                                    </td>
                                                                    <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                                                                        <?php echo $data[$counter]['Stock'];?>
                                                                    </td>
                                                                    </tr>
                                                               <?php
...
<script>
    function handle_search(my_input){
        var names = <?php echo json_encode($name); ?>;
        console.log(my_input);
        if (!names.includes(my_input.value)){
            my_input.value = "";
            my_input.placeholder='Member company non existant, check your spelling';
			}
        }
</script>
