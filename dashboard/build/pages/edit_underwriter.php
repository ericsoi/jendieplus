    ...
    if (isset($_SESSION["username"])){
        ...
        if($user){
            ...
            if($user_info->role == "admin" && $user_info->is_active==1){
                $underwriter=selectuser("SELECT * FROM tbl_underwriter WHERE ID='$ID'");
                ...
                if(isset($_POST["submit"])){
                    ...
                    $update = $pdo->prepare("UPDATE tbl_underwriter SET EMAIL_ADDRESS='$EMAIL_ADDRESS', EMAIL_CC='$EMAIL_CC', description='$description' WHERE ID='$ID'");
                    $update->execute();
                ...
                $EMAIL_ADDRESS=$underwriter->email;
                $description=$underwriter->description;
                $EMAIL_CC=$underwriter->email;
                if(isset($_POST["submit"])){
                    $EMAIL_ADDRESS = $_POST['email'];
                    $EMAIL_CC = $_POST['cc'];
                    $description = $_POST['desc'];
                    $update = $pdo->prepare("UPDATE tbl_email SET email='$EMAIL_ADDRESS',emailcc='$EMAIL_CC' WHERE id='$ID'");
                    $update->execute();
                    if($update->rowCount() > 0){
                    }elseif($update->execute()){
                    }
