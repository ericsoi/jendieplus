...
<div class="flex space-x-2 absolute inset-x-0 top-0 h-16">
    <?php
        if(isset($status["status"])){
            $state = $status["status"];
        }if(isset($status["benefit_name"])){
            $message = "Benefit ". str_replace("_", " ", strtolower($status["benefit_name"]));
        }
    ?>
    <?php if($state=="exists"){?>
        ...
        <div class="ml-3 text-sm font-normal"><?php echo $message . " " . $state;?></div>
        <button type="button" id="close_toast" class="ml-auto -mx-1.5 -my-1.5 bg-white text-gray-400 hover:text-gray-900 rounded-lg focus:ring-2 focus:ring-gray-300 p-1.5 hover:bg-gray-100 inline-flex h-8 w-8 dark:text-gray-500 dark:hover:text-white dark:bg-gray-800 dark:hover:bg-gray-700" data-dismiss-target="#toast-warning" aria-label="Close">
