<?php
	...
	if(isset($_POST["btn_login"])){
		...
		if($total_records < 1){
			header ("Location: ../build/pages/reset.php?status=none");
		}else{
			$update = $pdo->prepare("UPDATE tbl_user SET password='$password' WHERE phonenumber='$phonenumber'");
			$update->execute();
			echo $update->rowCount();
			if($update->rowCount() > 0){
				header ("Location: ../build/pages/reset.php?status=success");
			}else{
				header ("Location: ../build/pages/reset.php?status=error");
			}
		}
		// session_start();
		// include "../db/connect_db.php";
