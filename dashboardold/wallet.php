              ...
              <div class="row">
                ...
                <br>
                  ...
                  #print_r($_SESSION);
                    ...
                    $select = $pdo->prepare("SELECT * from tbl_user where agent = '$owner'");
                    // WHERE order_date BETWEEN :fromdate AND :todate");
                    $select->bindParam(':time_created', $_POST['date_1']);
                    $select->bindParam(':time_created', $_POST['date_2']);
                    $select->execute();

                    $row = $select->fetch(PDO::FETCH_OBJ);
                    if(isset($row->commission_value)){
                      $total = $row->commission_value;
                      $invoice = $row->commission_value;
                    }elseif(isset($row->commission_percentage)){
                      $total = $row->commission_percentage * $row->wallet;
                      $invoice = $row->commission_percentage * $row->wallet;
                    }else{
                      $total = 0;
                      $invoice = 0;
                    }
                  ...
                  ?>
             <div class="d-flex justify-content-center"><button type="button" class="btn-lg btn-primary">Withdraw</button></div>
                ...
                <div class="col-md-offset-1 col-md-5 col-xs-12">
                  <div class="info-box">
                    ...
                    <div class="info-box-content">
                      ...
                      <span class="info-box-number">Ksh.<?php echo number_format(0) ; ?></span>
                    </div>
                    <!-- /.info-box-content -->
                  </div>
                  <!-- /.info-box -->
